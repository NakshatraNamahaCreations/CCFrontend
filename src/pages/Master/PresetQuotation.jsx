...
const PresetQuotation = () => {
  ...
  const loadPresetQuotations = async (page = 1, searchValue = "") => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/preset-quotation?page=${page}&limit=${itemsPerPage}&search=${encodeURIComponent(searchValue)}`
      );
      setPresetQuotations(response.data.data);
      setTotalPages(response.data.pages || 1);
    } catch (err) {
      toast.error(err.response?.data?.message || "Failed to fetch preset quotations");
    }
  };
  ...
  const handleDelete = async (id) => {
    try {
      ...
      toast.error(err.response?.data?.message || "Failed to delete preset quotation");
    }
  };
              ...
              {presetQuotations.length > 0 ? (
                presetQuotations.map((preset, index) => (
                  <tr key={preset._id} className="text-center">
                    ...
                    <td className="text-start" style={{ fontSize: "12px" }}>
                      {preset.category} - [{preset.services.map((s) => s.serviceName).join(", ")}]
                    </td>
                    <td className="text-start" style={{ fontSize: "12px" }}>
                ...
                <tr>
                  <td colSpan="6" className="text-center py-3">
                    No preset quotations found. Add a new preset using the button above.
                  </td>
                </tr>
              )}
            </tbody>
