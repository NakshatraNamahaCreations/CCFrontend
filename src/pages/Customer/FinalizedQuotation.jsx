...
//     message += `*Date:* ${
//       quotation.packages?.[0]?.eventStartDate || "N/A"
//     }\n\n`;
...
//       message += `Installment ${index + 1}: ${inst.paymentPercentage}% (₹${
//         inst.paymentAmount !== undefined && inst.paymentAmount !== null
...
//   const packageSubtotal = (quotation.packages || []).reduce(
//     (sum, pkg) =>
//       sum +
//       (pkg.services || []).reduce(
//         (s, srv) => s + Number(srv.price || 0) * Number(srv.qty || 1),
//         0
//       ),
//     0
//   );
//   const discount = Number(quotation.discountValue || 0);
//   const gst = Number(quotation.gstValue || 0);
...
//           onClick={() =>
//             navigate(
//               `/customer/create-quote/${quotation.leadId?._id || ""}/${
//                 quotation.queryId
//               }`,
//               { state: { selectQuotationId: quotation._id } }
//             )
//           }
//         >
...
//               <p className="mb-1">
//                 <strong>Name:</strong> {leadId?.persons?.[0]?.name || "N/A"}
//               </p>
//               <p className="mb-1">
//                 <strong>Phone:</strong> {leadId?.persons?.[0]?.phoneNo || "N/A"}
//               </p>
//               <p className="mb-1">
//                 <strong>Email:</strong> {leadId?.persons?.[0]?.email || "N/A"}
//               </p>
//               <p className="mb-1">
//                 <strong>Whatsapp:</strong>{" "}
//                 {leadId?.persons?.[0]?.whatsappNo ||
//                   leadId?.persons?.[0]?.phoneNo ||
//                   "N/A"}
//               </p>
//               <p className="mb-1">
//                 <strong>Reference:</strong> {leadId?.referenceForm || "N/A"}
//               </p>
//             </Col>
...
//               </p>
//               <p className="mb-1">
//                 <strong>Title:</strong> {quotation.quoteTitle}
//               </p>
//               <p className="mb-1">
//                 <strong>Description:</strong> {quotation.quoteDescription}
//               </p>
//               <p className="mb-1">
...
//           })}
//           {/* Grand Total */}
//           <div className="bg-light p-3 fw-bold border">
//             <div className="mb-0 d-flex justify-content-between">
//               <h5>Total Package Amount:</h5>
//               <h5>₹{packageSubtotal.toLocaleString()}</h5>
//             </div>
//             <div className="mb-0 d-flex justify-content-between">
//               <h5>Discount:</h5>
//               <h5>- ₹{discount.toLocaleString()}</h5>
//             </div>
//             <div className="mb-0 d-flex justify-content-between">
//               <h5 className="mb-0">GST 18%</h5>
//               <h5>₹{gst.toLocaleString()}</h5>
//             </div>
//             <hr />
...
//                 <td>
//                   {(installments || []).reduce(
//                     (sum, inst) => sum + (inst.paymentPercentage || 0),
//                     0
//                   )}
//                   %
//                 </td>
...
//             </li>
//             <li className="mb-2">
//               A booking fee of {installments[0]?.paymentPercentage || 30}% is
...
const FinalizedQuotation = () => {
  ...
  // This useMemo MUST live before any early return:
    ...
    try {
      await toast.promise(
        (async () => {
          ...
          const margin = 5; // mm total margin on each side
  ...
  const handleShareWhatsApp = () => {
    ...
    if (
      ...
      Number(quotation?.totalPackageAmt) > 0
    ) {
      message += `\n*Package Details:*\n`;
      (quotation?.packages || []).forEach((pkg, index) => {
        message += `\n${index + 1}. *${pkg.categoryName}*\n`;
    ...
    if ((quotation?.albums || []).length) {
      message += `\n*Albums:*\n`;
      (quotation.albums || []).forEach((a, i) => {
        const albumName = a?.snapshot?.templateLabel || a?.templateId;
        const boxName = a?.snapshot?.boxLabel || a?.boxTypeId;
        const qty = a?.qty || 1;
  ...
  return (
    <>
      ...
      <div className="container py-4" style={{ fontSize: "0.85rem" }}>
        ...
        {/* Action Buttons */}
        <div className="d-flex justify-content-end mt-4">
          {q?.finalized && q?.bookingStatus !== "Booked" && (
            <Button
              variant="success"
              className="px-4"
              onClick={() => navigate(`/payment/installment-details/${q._id}`)}
            >
              Confirm Booking
            </Button>
          )}
