...
const VendorDetails = () => {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        if (res.data.success) {
          setSpecializationOptions(res.data.data.map(service => ({
            name: service.name,
            id: service._id,
            price: service.price,
  ...
  useEffect(() => {
    if (vendorData) {
      setVendorDetails({
        ...
        designation: vendorData.designation || (Array.isArray(vendorData.services) ? vendorData.services[0]?.name : "") || "",
      ...
      setVendorCat(vendorData.category === "Inhouse Vendor" ? "In house" : "Outsource");
    } else {
      setVendorDetails((prev) => ({
        ...prev,
        category: vendorCat === "In house" ? "Inhouse Vendor" : "Outsource Vendor",
      }));
    }
  ...
  const selectedSpecializationObjects = Array.isArray(vendorDetails.specialization)
  ...
  const handleEquipmentSelect = (selectedList) => {
    setSelectedEquipment(selectedList.map(eq => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        otherEquipment: vendorCat === "Outsource" ? vendorDetails.otherEquipment : undefined,
        equipmentDetails: selectedEquipment && selectedEquipment.length > 0 ? selectedEquipment : undefined,
        bankDetails: {
      ...
      if (response.data.success) {
        toast.success(id !== "new" ? "Vendor updated successfully" : "Vendor added successfully");
      ...
      toast.error(error.response?.data?.message || error.response?.data?.error || "Failed to save vendor");
    }
  };

  return (
    <div className="container py-2 rounded" style={{ background: "#F4F4F4", fontSize: "14px" }}>
          ...
          <div className="row">
            <div className="col-md-6">
              <Form.Group>
                ...
                <Form.Control type="text" value={vendorDetails.name} onChange={(e) => handleInputChange("name", e.target.value)} required />
              </Form.Group>
            </div>
            <div className="col-md-6">
              <Form.Group>
            ...
            <div className="col-md-6">
              <Form.Group>
                ...
                <Form.Control
                  ...
                  maxLength={10}
                  onChange={e => handleInputChange("alternatePhoneNo", e.target.value.replace(/\D/g, "").slice(0, 10))}
                />
              </Form.Group>
            </div>
            <div className="col-md-6">
              <Form.Group>
                ...
                <Form.Control type="email" value={vendorDetails.email} onChange={(e) => handleInputChange("email", e.target.value)} required />
              </Form.Group>
            </div>
          </div>
          <div className="row mt-2">
            <div className="col-md-6">
              <Form.Group>
                ...
                <Form.Control type="text" value={vendorDetails.address} onChange={(e) => handleInputChange("address", e.target.value)} required />
              </Form.Group>
            </div>
            <div className="col-md-6">
              <Form.Group>
                <Form.Label>Experience</Form.Label>
                <Form.Control type="text" value={vendorDetails.experience} onChange={(e) => handleInputChange("experience", e.target.value)} />
              </Form.Group>
            </div>
          </div>
          <div className="row mt-2">
            <div className="col-md-6">
              <Form.Group>
                ...
                <Form.Control type="text" value={vendorDetails.designation} onChange={(e) => handleInputChange("designation", e.target.value)} />
              </Form.Group>
            </div>
            <div className="col-md-6">
              <Form.Group>
                <Form.Label>Expertise Level</Form.Label>
                <Form.Select value={vendorDetails.expertiseLevel} onChange={(e) => handleInputChange("expertiseLevel", e.target.value)}>
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="mb-2 d-flex align-items-center gap-2">
                        <label htmlFor={`qty-${eq.id}`} style={{ minWidth: 70, marginBottom: 0 }}>Quantity:</label>
                        <input
                          id={`qty-${eq.id}`}
                          type="text"
                          inputMode="numeric"
                          ...
                          onChange={e => {
                            ...
                            setSelectedEquipment(prev => prev.map((item, i) =>
                              i === idx
                                ? { ...item, qty: val === '' ? '' : parseInt(val, 10), models: Array(val === '' ? 1 : parseInt(val, 10)).fill(item.models[0] || ''), sameModel: item.sameModel }
                                : item
                            ));
                          }}
                          onBlur={e => {
                            ...
                            if (e.target.value === '' || e.target.value === '0') {
                              setSelectedEquipment(prev => prev.map((item, i) =>
                                i === idx
                                  ? { ...item, qty: 1, models: Array(1).fill(item.models[0] || ''), sameModel: item.sameModel }
                                  : item
                              ));
                            }
                          }}
                          style={{ width: 80, borderRadius: 4, border: '1px solid #ced4da', padding: '2px 8px' }}
                        />
                      </div>
                      <div className="mb-2 d-flex align-items-center gap-2">
                        <input
                              ...
                              i === idx
                                ? {
                                  ...
                                  models: e.target.checked
                                    ? [item.models[0] || ""]
                                    : Array(item.qty === '' ? 1 : item.qty).fill(item.models[0] || "")
                                }
                                : item
                            ));
                          }}
                          style={{ marginRight: 6 }}
                        />
                        <label htmlFor={`sameModel-${eq.id}`} style={{ marginBottom: 0, userSelect: 'none' }}>All same model</label>
                      </div>
                      <div>
                        {eq.sameModel ? (
                          <div className="mb-2">
                            <label htmlFor={`model-${eq.id}-0`} style={{ minWidth: 90, marginBottom: 0 }}>Model Name:</label>
                            <input
                              id={`model-${eq.id}-0`}
                              type="text"
                              ...
                              style={{ width: 220, borderRadius: 4, border: '1px solid #ced4da', padding: '2px 8px' }}
                            />
                          </div>
                        ) : (
                          Array.from({ length: eq.qty === '' ? 1 : eq.qty }).map((_, i) => (
                            <div key={i} className="mb-2 d-flex align-items-center gap-2">
                              <label htmlFor={`model-${eq.id}-${i}`} style={{ minWidth: 110, marginBottom: 0 }}>Model Name {i + 1}:</label>
                              <input
                                id={`model-${eq.id}-${i}`}
                                type="text"
                                ...
                                value={eq.models[i] || ""}
                                onChange={e => {
                                  setSelectedEquipment(prev => prev.map((item, j) =>
                                    j === idx
                                      ? {
                                        ...item,
                                        models: item.models.map((m, mi) => mi === i ? e.target.value : m)
                                      }
                                      : item
                                  ));
                                }}
          ...
          <div className="row mt-2">
            <div className="col-md-6">
              <Form.Group>
                ...
                <Form.Control type="text" value={vendorDetails.bankDetails.bankName} onChange={(e) => handleBankInputChange("bankName", e.target.value)} required />
              </Form.Group>
            </div>
            <div className="col-md-6">
              <Form.Group>
                <Form.Label>Account Holder Name</Form.Label>
                <Form.Control type="text" value={vendorDetails.bankDetails.accountHolder} onChange={(e) => handleBankInputChange("accountHolder", e.target.value)} required />
              </Form.Group>
            </div>
          </div>
          <div className="row mt-2">
            <div className="col-md-6">
              <Form.Group>
                ...
                <Form.Control type="text" value={vendorDetails.bankDetails.accountNumber} onChange={(e) => handleBankInputChange("accountNumber", e.target.value)} required />
              </Form.Group>
            </div>
            <div className="col-md-6">
              <Form.Group>
                <Form.Label>IFSC Code</Form.Label>
                <Form.Control type="text" value={vendorDetails.bankDetails.ifsc} onChange={(e) => handleBankInputChange("ifsc", e.target.value)} required />
              </Form.Group>
            </div>
          </div>
          <div className="row mt-2">
            <div className="col-md-6">
              <Form.Group>
                ...
                <Form.Control type="text" value={vendorDetails.bankDetails.branch} onChange={(e) => handleBankInputChange("branch", e.target.value)} required />
              </Form.Group>
            </div>
            <div className="col-md-6">
              <Form.Group>
                <Form.Label>PAN Number (Optional)</Form.Label>
                <Form.Control type="text" value={vendorDetails.bankDetails.panNumber} onChange={(e) => handleBankInputChange("panNumber", e.target.value.toUpperCase())} pattern="^[A-Z]{5}[0-9]{4}[A-Z]{1}$" title="Example: ABCDE1234F" />
              </Form.Group>
            </div>
            <div className="col-md-6">
            <Form.Group>
            <Form.Label>Aadhar Number </Form.Label>
              <Form.Control
                ...
                value={
                  showAadhaarMasked
                    ? vendorDetails.bankDetails.aadhaarNumber.replace(/\d(?=\d{4})/g, "*")
