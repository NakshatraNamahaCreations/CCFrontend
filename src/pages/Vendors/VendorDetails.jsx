...
//           // Only static Driver here
//           const staticSpecialization = [
//             { id: "static-driver", name: "Driver" },
//           ];
...
//         s.id === id || s.serviceId === id ? { ...s, salary } : s
//       ),
//     }));
//   };
...
//   const handleEquipmentSelect = (selectedList) => {
//     // Only update when new equipment is selected.
//     setSelectedEquipment(
//       selectedList.map((eq) => {
//         const existing = selectedEquipment.find((e) => e.id === eq.id);
//         return existing || { ...eq, qty: 1, models: [""], sameModel: false };
...
//     // This will update the state when equipment is removed.
...
//   //     const services = (vendorDetails.specialization || []).map((service) => {
//   //       return {
//   //         serviceId: service.id?.startsWith("static-")
//   //           ? null // 👈 Driver has no ObjectId
...
//   //       email: vendorDetails.email,
//   //       address: vendorDetails.address,
//   //       services: vendorDetails.specialization.map((service) => ({
//   //         serviceId: service.serviceId || null, // Handle cases for static services like Driver
//   //         name: service.name,
//   //         salary: service.salary || undefined, // Only include salary if defined
//   //       })),
//   //       experience: vendorDetails.experience,
//   //       designation: vendorDetails.designation,
//   //       expertiseLevel: vendorDetails.expertiseLevel,
...
//   //       otherEquipment:
//   //         vendorCat === "Outsource" ? vendorDetails.otherEquipment : undefined,
//   //       equipmentDetails: selectedEquipment.length > 0 ? selectedEquipment : [], // Ensure this is passed
//   //       bankDetails: {
//   //         ...vendorDetails.bankDetails,
//   //         panNumber: vendorDetails.bankDetails.panNumber || undefined,
//   //       },
...
//   try {
//     // Process the specialization (services) array
//     const services = vendorDetails.specialization.map(service => ({
//       serviceId: service.id?.startsWith("static-") ? null : service.serviceId || service.id || service._id,
//       name: service.name,
//       ...(service.salary ? { salary: Number(service.salary) } : {}),
//     }));
...
//       designation: vendorDetails.designation,
//       expertiseLevel: vendorDetails.expertiseLevel,
//       specialization: services, // List of services (specializations)
//       camera: vendorCat === "Outsource" ? vendorDetails.camera : undefined,
//       otherEquipment: vendorCat === "Outsource" ? vendorDetails.otherEquipment : undefined,
//       equipmentDetails: selectedEquipment.length > 0 ? selectedEquipment : [], // Ensure equipment details are included
//       bankDetails: {
//         ...vendorDetails.bankDetails,
//         panNumber: vendorDetails.bankDetails.panNumber || undefined, // Optional field for PAN
//       },
//     };

//     // Clean the payload by removing undefined fields
//     Object.keys(payload).forEach(key => payload[key] === undefined && delete payload[key]);

//     // Send the request to the backend to add or update the vendor
...
//       localStorage.removeItem("selectedCategory"); // Clean up localStorage after success
//       navigate("/vendors"); // Navigate to the vendor list page
//     } else {
//       toast.error("Failed to save vendor"); // Show error message
//     }
//   } catch (error) {
...
const VendorDetails = () => {
 ...
 const handleSubmit = async (e) => {
  ...
  if (!vendorDetails.specialization || vendorDetails.specialization.length === 0) {
  ...
  try {
    ...
    if (response.data.success) {
      toast.success(
        id !== "new" ? "Vendor updated successfully" : "Vendor added successfully"
      );
      localStorage.removeItem("selectedCategory");
      navigate("/vendors");
    } else {
