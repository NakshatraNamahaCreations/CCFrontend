...
const VendorManagement = () => {
  ...
  const handleDeleteVendor = async (vendorId) => {
    if (window.confirm("Are you sure you want to delete this vendor?")) {
      try {
        const response = await axios.delete(`http://localhost:5000/api/vendors/${vendorId}`);
              ...
              {vendors.length === 0 ? (
                ...
                vendors.map((vendor, index) => (
                  ...
                  >
                    ...
                    <td>{vendor.name}</td>
                    <td>{vendor.category === "Inhouse Vendor" ? "In house" : "Outsource"}</td>
                    <td>{vendor.phoneNo}</td>
                    <td>{Array.isArray(vendor.services) && vendor.services.length > 0 
  ? vendor.services.map((s, i, arr) => (
      <React.Fragment key={s.serviceId || s.id || s._id || i}>
        {s.name}{i < arr.length - 1 ? ', ' : ''}{i < arr.length - 1 ? <br /> : null}
      </React.Fragment>
    ))
  : "N/A"}
</td>
